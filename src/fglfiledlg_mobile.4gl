...
PRIVATE FUNCTION expand_dirs(p)
    ...
    IF p == 0 THEN
        LET parent_path = ""
    ELSE
        LET parent_path = tree_arr[p].id --the dir being expanded becomes the parent, id
    ...
    WHILE TRUE
        ...
        EXIT WHILE
        END IF
        IF fname == "." OR fname == ".." THEN --skip any current dir or parent dir stuff
        CONTINUE WHILE
        END IF


        LET fullpath = os.Path.join(IIF(p == 0, ".", parent_path), fname) --construct full path
        ...
        LET i = i + 1 --move to next idx and then insert the subdir into tree, while assigning all of its fields
...
PRIVATE FUNCTION collapse_dirs(p)
    ...
    WHILE p < tree_arr.getLength()
     IF tree_arr[p + 1].parentid IS NULL OR tree_arr[p + 1].parentid != parent_path THEN
    EXIT WHILE
    END IF
...
PRIVATE FUNCTION hasChildDirs(dirpath)
    ...
    IF dh == 0 THEN RETURN FALSE END IF
...
PUBLIC FUNCTION browse() ------------------keep
  ...
  CALL list_dir(os.Path.pwd(),os.Path.pwd()) 
  RETURNING retstr
...
#
   FUNCTION sync_list_with_tree(dlg, tree_idx)
    ...
    LET new_path = tree_arr[tree_idx].id --get path of the node selected by going into the correct index in the tree array and getting its id
    let currpath = new_path
    CALL _filedlg_fetch_filenames(dlg, new_path, ftype, NULL) --fills the list view with what is in the dir
